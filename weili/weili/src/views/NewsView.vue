<template>
    <div>
        ...
        <div class="main center">
            <div class="main_tit">
                <h2>新闻资讯</h2>
                <img src="http://www.jxwlrl.com/images/jian.png" alt="">
                <p>提供有价值的人力资源服务和解决方案，持续为客户创造价值</p>
                <ul>
                    <li v-for="item in sortclass" ><router-link :to="{ path: '/news', query: { id: item.id } }" :class="{ on: sortid == item.id }">{{ item.sortname
                    }}</router-link></li>
                </ul>
            </div>
            <div class="qzzp_ms_list">
                <ul>
                    <li v-for="item in news">
                        ...
                        <router-link :to="{ name: 'NewsDetailView', params: { id: item.id } }">
                            <h2>{{ item.title }}</h2>
                            <p>{{ item.desc }}</p>
                        </router-link>
                    </li>
